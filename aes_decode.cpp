...
//逆列混合矩阵
static const int incol[4][4] = { 0xe, 0xb, 0xd, 0x9,
	0x9, 0xe, 0xb, 0xd,
	0xd, 0x9, 0xe, 0xb,
	0xb, 0xd, 0x9, 0xe };
uc key[4][4]={
...
int main()
{
	...
	for (int i=0;i<n;i+=16)
	{
		...
		for (int x=0;x<4;x++)
			for (int y=0;y<4;y++)
				f[x][y]=f[x][y]^nkey[x][y];
				
		//逆序操作 
		for (int T=round-1;T>=1;T--)
		{
			//S盒转换 
			for (int x=0;x<4;x++)
				for (int y=0;y<4;y++)
					f1[x][y]=inS[(f[x][y]>>4)][f[x][y]&(15)];
			//行移位 
			for (int x=0;x<4;x++)
				for (int y=0;y<4;y++)
					f2[x][y]=f1[x][(y-x+4)%4];
